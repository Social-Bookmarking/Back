...
public class CommentController {
    ...
    public ResponseEntity<CursorResultDto<CommentDto.TopLevelResponseDto>> getTopLevelComments(
            @Parameter(description = "댓글을 조회할 북마크 ID") @PathVariable("bookmarkId") Long bookmarkId,
            @RequestParam(required = false) Long cursor,
            @RequestParam(defaultValue = "10") int size){
        CursorResultDto<CommentDto.TopLevelResponseDto> comments = commentService.getTopLevelComments(bookmarkId, cursor, size);
    ...
    public ResponseEntity<CursorResultDto<CommentDto.ReplyResponseDto>> getReplies(
            @Parameter(description = "답글을 조회할 댓글 ID") @PathVariable("commentId") Long commentId,
            @RequestParam(required = false) Long cursor,
            @RequestParam(defaultValue = "10") int size) {
        CursorResultDto<CommentDto.ReplyResponseDto> replies = commentService.getReplyComments(commentId, cursor, size);
